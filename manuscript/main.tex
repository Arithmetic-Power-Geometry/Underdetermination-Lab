...
The proposed framework was formulated mathematically and then implemented as a reproducible software artifact. The repository contains the statistical admission layer, obstruction logic, one-step and sequential planning components, the exact target-aware dynamic-programming baseline, the controlled ML deployment case, automated tests, and the GitHub Actions workflow used to regenerate the reported numerical results.

The implementation serves as an executable validation of the mathematical formulation. In particular, the exact dynamic-programming baseline reproduces the same finite planning optimum as the sequential wrapper on the matched benchmark, so the planning layer is treated as an existing downstream component rather than as a claimed methodological novelty.
...
With $K=17/20$ and $\alpha=0.05$, the 0.01 grid admits 33 worlds from 0.63 through 0.95. A downstream threshold $d=0.80$ therefore remains unresolved. The endpoints 0.63 and 0.95 form an easy contradiction-exposure pair, whereas 0.79 and 0.80 are the closest finite-grid cross-decision pair; \Cref{fig:admissible} visualizes the resulting admissible interval and boundary location.
...
As summarized by \Cref{alg:dp}, the exact baseline returns the same worst-case cost 112,402.5 and the same first experiment. This result is important precisely because it removes a tempting but incorrect novelty claim. Once the finite world table is supplied, our planning wrapper is subsumed on this benchmark by exact target-aware planning.
...
\node[box] (K) {Finite noisy evidence\\$K$};
\node[box,right=of K] (C) {Admission\\$\CalC_\alpha(K)$};
\node[box,right=of C] (D) {Decision map\\$d(W)$};
\node[box,below=of D] (P) {Target-aware planner\\existing / interchangeable};
\node[box,left=of P] (E) {Experiment\\least-cost feasible};
\node[box,right=of P] (O) {Typed obstruction\\family / margin / budget};
\draw[arr] (K) -- (C);
\draw[arr] (C) -- (D);
\draw[arr] (D) -- node[right,font=\scriptsize] {disagreement} (P);
\draw[arr] (P) -- node[above,font=\scriptsize] {feasible} (E);
\draw[arr] (P) -- node[above,font=\scriptsize] {infeasible} (O);
\draw[arr] (D.west) to[bend left=35] node[below,font=\scriptsize] {agreement / stop} (K.south);
\end{tikzpicture}
...
\section{Conclusion}
This study introduces an evidence-first protocol rather than a new universal experimental-design algorithm. The procedure constructs coverage-controlled admissible worlds from finite noisy data, determines whether those worlds agree on the downstream decision, passes unresolved cases to a cost-aware target-identification planner, and returns either a least-cost feasible experiment or a typed statistical obstruction. In the controlled Bernoulli and ML cases, this separation prevents easy contradictions from being confused with hard decision boundaries and prevents a finite-grid cost from being misrepresented as a continuous guarantee. The exact baseline further shows that scientific value can come from identifying the right interface and stopping rule even when the downstream optimizer is already known.
